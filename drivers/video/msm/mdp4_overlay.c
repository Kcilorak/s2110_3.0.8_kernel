...
void mdp4_set_perf_level(void)
{
	...
	if (mdp4_extn_disp || camera_enable)
...
#ifdef CONFIG_WESTLAKE
		if (widevine_secure_section() == 1 &&
			(req->src_rect.w > mfd->panel_info.xres ||
			 req->src_rect.h > mfd->panel_info.yres) &&
			((req->src_rect.h > (2 * req->dst_rect.h)) ||
