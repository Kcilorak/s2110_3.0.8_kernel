...
/* Adjust backlight brightness definition */
#define MSM_FB_MAX_DBGFS 1024
#define MAX_BACKLIGHT_BRIGHTNESS 255
#define HALF_BACKLIGHT_BRIGHTNESS 127
#define MIN_BACKLIGHT_BRIGHTNESS 30
#define ADJUST_HALF_BACKLIGHT_BRIGHTNESS 102
#define ADJUST_MIN_BACKLIGHT_BRIGHTNESS 5
					...
					enum led_brightness value)
{
	...
	/* Adjust backlight brightness method */
	if (value > HALF_BACKLIGHT_BRIGHTNESS)
		adj_value = (((MAX_BACKLIGHT_BRIGHTNESS - ADJUST_HALF_BACKLIGHT_BRIGHTNESS) * value)
					+ (ADJUST_HALF_BACKLIGHT_BRIGHTNESS - HALF_BACKLIGHT_BRIGHTNESS) * MAX_BACKLIGHT_BRIGHTNESS)
			        / (MAX_BACKLIGHT_BRIGHTNESS - HALF_BACKLIGHT_BRIGHTNESS);
	else if (value > MIN_BACKLIGHT_BRIGHTNESS)
		adj_value = (((ADJUST_HALF_BACKLIGHT_BRIGHTNESS - ADJUST_MIN_BACKLIGHT_BRIGHTNESS) * value)
					+ (ADJUST_MIN_BACKLIGHT_BRIGHTNESS * HALF_BACKLIGHT_BRIGHTNESS - ADJUST_HALF_BACKLIGHT_BRIGHTNESS * MIN_BACKLIGHT_BRIGHTNESS))
			        / (HALF_BACKLIGHT_BRIGHTNESS - MIN_BACKLIGHT_BRIGHTNESS);
	else
		adj_value = (ADJUST_MIN_BACKLIGHT_BRIGHTNESS * value) / MIN_BACKLIGHT_BRIGHTNESS;

	/* This maps android backlight level 0 to 255 into
	   driver backlight level 0 to bl_max with rounding */
	bl_lvl = (2 * adj_value * mfd->panel_info.bl_max + MAX_BACKLIGHT_BRIGHTNESS)
...
#ifdef CONFIG_FB_MSM_LOGO
	if (mfd->index == 0 && get_silent_reset_status() == 0) {
		if (!load_565rle_image(INIT_IMAGE_FILE)) {	/* Flip buffer */
			...
			MSM_FB_ERR("Cannot open initlogo.rle\n");
	}
#endif
	ret = 0;
