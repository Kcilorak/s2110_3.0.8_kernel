...
static ssize_t
show_sec_hw_key_blown_check(struct device *dev, struct device_attribute *attr, char *buf)
{
	    uint32 HW_KEY_STATUS;
		    HW_KEY_STATUS = inpdw(QFPROM_BASE + 0x00002050) & 0x1;
			    return snprintf(buf, PAGE_SIZE, "%d\n", HW_KEY_STATUS);
}

static DEVICE_ATTR(sec_hw_key_blown_check, S_IRUGO,
                   show_sec_hw_key_blown_check, NULL);

static ssize_t
show_secure_boot_enable(struct device *dev, struct device_attribute *attr, char *buf)
{
    uint32 QFPROM_RAW_OEM_SEC_BOOT_ROW0_LSB;
	QFPROM_RAW_OEM_SEC_BOOT_ROW0_LSB = inpdw(QFPROM_BASE + 0x00000310);
	if((QFPROM_RAW_OEM_SEC_BOOT_ROW0_LSB & 0x20) == 0x20)
	{
...
static DEVICE_ATTR(secure_boot_enable, S_IRUGO,
				   show_secure_boot_enable, NULL);

static ssize_t
show_jtag_id_check(struct device *dev, struct device_attribute *attr, char *buf)
{
    uint32 JTAG_ID;
	JTAG_ID = inpdw(QFPROM_BASE + 0x000060E0);
    return snprintf(buf, PAGE_SIZE, "0x%x\n", JTAG_ID);
}

static DEVICE_ATTR(jtag_id_check, S_IRUGO,
				   show_jtag_id_check, NULL);

static ssize_t
show_fuse_blown_check(struct device *dev, struct device_attribute *attr, char *buf)
{
	if(!memcmp(chip_id(), sbl_id(), 6))   /* just compare 6 bytes of JTAG_ID */
	{
		return snprintf(buf, PAGE_SIZE, "1\n");
	}
	else
...
static DEVICE_ATTR(fuse_blown_check, S_IRUGO,
				   show_fuse_blown_check, NULL);

static ssize_t
show_secure_boot_check(struct device *dev, struct device_attribute *attr, char *buf)
{
	uint32 QFPROM_RAW_OEM_CONFIG_ROW0_MSB, QFPROM_RAW_OEM_CONFIG_ROW2_LSB, QFPROM_RAW_OEM_SEC_BOOT_ROW0_LSB, JTAG_ID;
	QFPROM_RAW_OEM_CONFIG_ROW0_MSB = inpdw(QFPROM_BASE + 0x00000224);
	QFPROM_RAW_OEM_CONFIG_ROW2_LSB = inpdw(QFPROM_BASE + 0x00000230);
	QFPROM_RAW_OEM_SEC_BOOT_ROW0_LSB = inpdw(QFPROM_BASE + 0x00000310);
	JTAG_ID = inpdw(QFPROM_BASE + 0x000060E0);
	return snprintf(buf, PAGE_SIZE, "0224 => 0x%x, 0230 => 0x%x, 0310 => 0x%x, 60E0 => 0x%x\n", QFPROM_RAW_OEM_CONFIG_ROW0_MSB, QFPROM_RAW_OEM_CONFIG_ROW2_LSB, QFPROM_RAW_OEM_SEC_BOOT_ROW0_LSB, JTAG_ID);
}

static DEVICE_ATTR(secure_boot_check, S_IRUGO,
				   show_secure_boot_check, NULL);

static struct attribute *dev_attrs[] = {
	&dev_attr_secure_boot_check.attr,
	&dev_attr_fuse_blown_check.attr,
	&dev_attr_jtag_id_check.attr,
	&dev_attr_secure_boot_enable.attr,
	&dev_attr_sec_hw_key_blown_check.attr,
	NULL,
};
	...
	if (!hdmi_msm_state->hpd_cable_chg_detected) {
		...
		if (hpd_state) {
			...
			kobject_uevent(external_common_state->uevent_kobj,
						   KOBJ_ONLINE);
			/* Finish waiting works before HDCP authentication */
			msleep(200);
...
/*2011/12/23 Victor: add HDMI state for SMT build*/
#if defined(PEGA_SMT_BUILD)
static ssize_t check_hdmi_detect(struct device *dev,
...
static DEVICE_ATTR(hdmi_detect, S_IRUGO, check_hdmi_detect, NULL);

static struct attribute *hdmi_msm_attributes[] = {
	&dev_attr_hdmi_detect.attr,
	NULL
};

static struct attribute_group hdmi_msm_attribute_group = {
	.attrs = hdmi_msm_attributes
};
#endif
...
/*2011/12/23 Victor: add HDMI state for SMT build*/
#if defined(PEGA_SMT_BUILD)
rc = sysfs_create_group(&pdev->dev.kobj, &hdmi_msm_attribute_group);
if (rc) {
	DEV_DBG("%s cannot create attribute\n", __func__);
	return rc;
}
#endif
...
disable_irq(hdmi_msm_state->irq);
/* Jeremy1_Wang: Add attribute to read Fuse register value - Begin */
rc = sysfs_create_group(&pdev->dev.kobj, &dev_attr_grp);
if (rc) {
	DEV_ERR("Init FAILED: HDMI sysfs_create_group, rc=%d\n", rc);
	goto error;
}
/* Jeremy1_Wang: Add attribute to read Fuse register value - Emd */
