...
static int ami30x_set_bits8(void *mlsl_handle,
...
static int ami30x_wait_data_ready(void *mlsl_handle,
				  struct ext_slave_platform_data *pdata,
...
static int ami30x_read_raw_data(void *mlsl_handle,
				struct ext_slave_platform_data *pdata,
				short dat[3])
{
	int result;
...
static int ami30x_force_mesurement(void *mlsl_handle,
				   struct ext_slave_platform_data *pdata,
				   short ver[3])
{
	int result;
	int status;
	result = ami30x_set_bits8(mlsl_handle, pdata,
				  AMI_REG_CTRL3, AMI_CTRL3_FORCE_BIT);
	ERROR_CHECK(result);

	result = ami30x_wait_data_ready(mlsl_handle, pdata,
					AMI_DRDYWAIT, AMI_WAIT_DATAREADY_RETRY);
	if (result && result != INV_ERROR_COMPASS_DATA_OVERFLOW)
		ERROR_CHECK(result);
	/*  READ DATA X,Y,Z */
	status = ami30x_read_raw_data(mlsl_handle, pdata, ver);
	ERROR_CHECK(status);

	return result;
}

static int ami30x_mea(void *mlsl_handle,
		      struct ext_slave_platform_data *pdata, short val[3])
{
	int result = ami30x_force_mesurement(mlsl_handle, pdata, val);
	ERROR_CHECK(result);
...
static int ami30x_write_offset(void *mlsl_handle,
			       struct ext_slave_platform_data *pdata,
			       unsigned char *fine)
{
	int result = 0;
...
static int ami30x_start_sensor(void *mlsl_handle,
			       struct ext_slave_platform_data *pdata)
{
	int result = 0;
	unsigned char buf[3];
	struct ami30x_private_data *private_data = pdata->private_data;
...
#endif
	/* Step 4 */
	result = ami30x_write_offset(mlsl_handle, pdata, private_data->fine);
	ERROR_CHECK(result);
	return result;
}
 ...
 */
static int ami30x_read_param(void *mlsl_handle,
...
static int ami30x_initial_b0_adjust(void *mlsl_handle,
				    struct ext_slave_descr *slave,
				    struct ext_slave_platform_data *pdata)
{
	int result;
	...
	for (fn = 0; fn < AMI_FINE_MAX; ++fn) {	/* fine 0 -> 95 */
		...
		result = ami30x_write_offset(mlsl_handle, pdata, fine);
		ERROR_CHECK(result);

		result = ami30x_force_mesurement(mlsl_handle, pdata, data);
		ERROR_CHECK(result);
...
static int ami30x_search_offset(void *mlsl_handle,
				struct ext_slave_descr *slave,
				struct ext_slave_platform_data *pdata)
{
	int result;
	...
	while (run_flg[0] == 1 || run_flg[1] == 1 || run_flg[2] == 1) {
		...
		result = ami30x_mea(mlsl_handle, pdata, val);
		ERROR_CHECK(result);
...
static int ami30x_read_win(void *mlsl_handle,
			   struct ext_slave_descr *slave,
			   struct ext_slave_platform_data *pdata)
{
	int result = 0;
...
static int ami30x_init(void *mlsl_handle,
		       struct ext_slave_descr *slave,
		       struct ext_slave_platform_data *pdata)
{
	int result;
	struct ami30x_private_data *private_data;
	private_data = (struct ami30x_private_data *)
	    inv_malloc(sizeof(struct ami30x_private_data));
	if (!private_data)
	...
	ERROR_CHECK(result);
	/* Read Parameters */
	result = ami30x_read_param(mlsl_handle, slave, pdata);
	ERROR_CHECK(result);
	/* Read Window */
	result = ami30x_initial_b0_adjust(mlsl_handle, slave, pdata);
	ERROR_CHECK(result);
	result = ami30x_start_sensor(mlsl_handle, pdata);
	ERROR_CHECK(result);
	result = ami30x_read_win(mlsl_handle, slave, pdata);
	ERROR_CHECK(result);
...
static int ami30x_exit(void *mlsl_handle,
		       struct ext_slave_descr *slave,
		       struct ext_slave_platform_data *pdata)
{
...
static int ami30x_config(void *mlsl_handle,
			 struct ext_slave_descr *slave,
			 struct ext_slave_platform_data *pdata,
...
static int ami30x_get_config(void *mlsl_handle,
			     struct ext_slave_descr *slave,
			     struct ext_slave_platform_data *pdata,
			     struct ext_slave_config *data)
{
	...
	case MPU_SLAVE_SEARCHOFFSET:
		if (sizeof(AMI_WIN_PARAMETER) > data->len)
			ERROR_CHECK(INV_ERROR_INVALID_PARAMETER);
		if (data->apply) {
			result = ami30x_search_offset(mlsl_handle,
						      slave, pdata);
			ERROR_CHECK(result);
			/* Start sensor */
			result = ami30x_start_sensor(mlsl_handle, pdata);
	...
	case MPU_SLAVE_READWINPARAMS:
		if (sizeof(AMI_WIN_PARAMETER) > data->len)
			ERROR_CHECK(INV_ERROR_INVALID_PARAMETER);
		if (data->apply) {
			result = ami30x_initial_b0_adjust(mlsl_handle,
							  slave, pdata);
			ERROR_CHECK(result);
			/* Start sensor */
			result = ami30x_start_sensor(mlsl_handle, pdata);
...
struct ext_slave_read_trigger ami30x_read_trigger = {
	/*.reg              = */ AMI_REG_CTRL3,
...
struct ext_slave_descr ami30x_descr = {
	...
	/*.read             = */ ami30x_read,
	/*.config           = */ ami30x_config,
	/*.get_config       = */ ami30x_get_config,
	/*.name             = */ "ami30x",
