	...
	if (mpuirq_dev_data.irq) {
		...
		if (BIT_ACTL_LOW == ((mldl_cfg->pdata->int_config) & BIT_ACTL))
			...
			flags = IRQF_TRIGGER_RISING;
                printk(KERN_INFO "[GYRO] gpio request irq value = %d!\n",mpuirq_dev_data.irq);

                   // [2011/12/6] Jackal - porting westlake
		//res =  request_threaded_irq(mpuirq_dev_data.irq, NULL, mpuirq_handler, flags ,interface, &mpuirq_dev_data.irq);
		res =  request_irq(mpuirq_dev_data.irq, mpuirq_handler, IRQF_SHARED|flags, interface, &mpuirq_dev_data.irq);
                   // [2011/12/6] Jackal - porting westlake End
                printk(KERN_INFO "[GYRO] gpio request irq state %d!\n",res);
		if (res) {
