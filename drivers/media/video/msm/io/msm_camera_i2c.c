...
int32_t msm_camera_i2c_write_bytes(struct msm_camera_i2c_client *client,
	struct msm_camera_i2c_reg_conf *reg_conf_tbl, uint16_t num_byte)
{
	int32_t rc = -EFAULT;
	unsigned char buf[client->addr_type+num_byte];
	uint16_t len = 0, i = 0;
	...
	for (i = 0; i < num_byte; i++) {
		buf[i+len] = (uint8_t)((reg_conf_tbl+i)->reg_data);
		S_I2C_DBG("Byte %d: 0x%x\n", i+len, buf[i+len]);
		S_I2C_DBG("Data: 0x%x\n", (uint8_t)((reg_conf_tbl+i)->reg_data));
	}
	...
	enum msm_camera_i2c_data_type data_type)
{
	...
	S_I2C_DBG("%s addr: 0x%x reg_data: 0x%x data: 0x%x mask: 0x%x\n",
			__func__, addr, reg_data, data, mask);

	new_data = (reg_data & ~mask) | (data & mask);

	if (reg_data != new_data) {
		S_I2C_DBG("%s write: 0x%x\n", __func__, new_data);

		rc = msm_camera_i2c_write(client, addr, new_data, data_type);
		if (rc < 0)
...
/*BEGIN Chaoyen_Wu@pegatron[2012.3.8][modify i2c polling function]*/			
			rc = msm_camera_i2c_poll(client, reg_conf_tbl->reg_addr,
				reg_conf_tbl->reg_data, reg_conf_tbl->dt);
/*END Chaoyen_Wu@pegatron[2012.3.8][modify i2c polling function]*/
/* BEGIN Dom_Lin@pegatron [2011/02/06] [Add I2C delay function] */
		} else if (reg_conf_tbl->cmd_type == MSM_CAMERA_I2C_CMD_DELAY) {
			if (reg_conf_tbl->reg_data > 20)
				msleep(reg_conf_tbl->reg_data);
			else
				usleep_range(reg_conf_tbl->reg_data*1000,
							(reg_conf_tbl->reg_data+1)*1000);
			rc = 0;
/* END Dom_Lin@pegatron [2011/02/06] [Add I2C delay function] */
/* BEGIN Dom_Lin@pegatron [2012/02/13] [Add I2C write with mask function] */
		} else if (reg_conf_tbl->cmd_type == MSM_CAMERA_I2C_CMD_WRITE_WITH_MASK) {
			...
			case MSM_CAMERA_I2C_BYTE_DATA:
			case MSM_CAMERA_I2C_WORD_DATA:
				rc = msm_camera_i2c_write_with_mask(
					client,
					reg_conf_tbl->reg_addr,
					reg_conf_tbl->reg_data,
					reg_conf_tbl->reg_mask,
					dt);
				break;
			default:
				pr_err("%s: Unsupport data type: %d\n",
...
/* BEGIN Dom_Lin@pegatron [2012/02/12] [Add I2C skip function] */
		} else if (reg_conf_tbl->cmd_type != MSM_CAMERA_I2C_CMD_NULL) {
/* END Dom_Lin@pegatron [2012/02/12] [Add I2C skip function] */
			...
			case MSM_CAMERA_I2C_UNSET_WORD_MASK:
				...
				break;
/* BEGIN Dom_Lin@pegatron [2012/03/14] [Tuning back camera I2C write for reducing open camera time] */
			case MSM_CAMERA_I2C_BYTE_DATA_AUTO:
				for (count = 1; i + count < size; ++count) {
					if ((reg_conf_tbl+count)->cmd_type != MSM_CAMERA_I2C_CMD_WRITE)
						break;
					if ((reg_conf_tbl+count)->reg_addr != reg_conf_tbl->reg_addr + count)
						break;
					if (count == 100) {
						++count;
						break;
				...
				if (count == 1) {
					rc = msm_camera_i2c_write(
						client,
						reg_conf_tbl->reg_addr,
						reg_conf_tbl->reg_data, MSM_CAMERA_I2C_BYTE_DATA);
				}
				else {
					rc = msm_camera_i2c_write_bytes(
						client,
						reg_conf_tbl,
						count);
					i = i + count - 1;
					reg_conf_tbl = reg_conf_tbl + count - 1;
				}
				break;
/* END Dom_Lin@pegatron [2012/03/14] [Tuning back camera I2C write for reducing open camera time] */
			default:
