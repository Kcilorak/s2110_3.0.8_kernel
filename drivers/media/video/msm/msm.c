...
static int msm_close(struct file *f)
{
	...
	if (pcam_inst->streamon) {
		...
		if (pcam->mctl.mctl_release) {
			pr_err("[%s: %d] handle mediaserver crash elegantly\n", __func__, __LINE__);
...
/* BEGIN Dom_Lin@pegatron [2012/04/12] [Add suspend wake lock] */
		wake_lock_destroy(&sync->suspend_wake_lock);
...
/* BEGIN Dom_Lin@pegatron [2012/04/12] [Add suspend wake lock] */
	wake_lock_init(&sync->suspend_wake_lock, WAKE_LOCK_SUSPEND, "msm_camera");
/* END Dom_Lin@pegatron [2012/04/12] [Add suspend wake lock] */
	sync->opencnt = 0;
