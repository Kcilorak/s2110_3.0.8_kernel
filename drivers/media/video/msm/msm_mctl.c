...
#undef D
#define D(fmt, args...) printk(KERN_DEBUG "[%s: %d] msm_mctl: " fmt, __func__, __LINE__, ##args)
#else
...
/* BEGIN Dom_Lin@pegatron [2012/03/08] [Enable back camera auto focus] */
static int msm_mctl_sensor_notify(struct v4l2_subdev *sd,
	unsigned int notification, void *arg)
{
	int rc = 0;
	struct v4l2_event v4l2_evt;
	struct msm_isp_event_ctrl *isp_event;
	struct msm_sync *sync = (struct msm_sync *)v4l2_get_subdev_hostdata(sd);
	struct msm_cam_media_controller *pmctl = &sync->pcam_sync->mctl;

	struct sensor_msg_stats *isp_stats = (struct sensor_msg_stats *)arg;
	uint8_t *stats_buf;
	...
	if (!isp_event) {
		pr_err("%s Insufficient memory for isp_event. return", __func__);
		return -ENOMEM;
	}

	stats_buf = kmalloc(sizeof(uint8_t), GFP_ATOMIC);
	if (!stats_buf) {
		pr_err("%s Insufficient memory for stats_buf. return", __func__);
		return -ENOMEM;
	}

	v4l2_evt.type = V4L2_EVENT_PRIVATE_START + MSM_CAM_RESP_S_STAT_EVT_MSG;
	*((uint32_t *)v4l2_evt.u.data) = (uint32_t)isp_event;

	isp_event->resptype = MSM_CAM_RESP_S_STAT_EVT_MSG;
	isp_event->isp_data.isp_msg.type = MSM_CAMERA_MSG;
	isp_event->isp_data.isp_msg.msg_id = isp_stats->id;
	isp_event->isp_data.isp_msg.frame_id = 0;

	*stats_buf = isp_stats->status;
	isp_event->isp_data.isp_msg.len	= sizeof(uint8_t);
	isp_event->isp_data.isp_msg.data = stats_buf;

	v4l2_event_queue(pmctl->config_device->config_stat_event_queue.pvdev, &v4l2_evt);

	D("---\n");
	return rc;
}
/* END Dom_Lin@pegatron [2012/03/08] [Enable back camera auto focus] */
	...
	unsigned int notification, void *arg)
{
	...
	case NOTIFY_CSIC_CFG:
		...
		break;
/* BEGIN Dom_Lin@pegatron [2012/03/08] [Enable back camera auto focus] */
	case NOTIFY_SENSOR_MSG_STATS:
		D("NOTIFY_SENSOR_MSG_STATS\n");
		if (p_mctl->isp_sdev) {
			rc = msm_mctl_sensor_notify(p_mctl->isp_sdev->sd, notification, arg);
		}
		break;
/* END Dom_Lin@pegatron [2012/03/08] [Enable back camera auto focus] */
	default:
			...
			unsigned int cmd, unsigned long arg)
{
	...
	if (cmd != MSM_CAM_IOCTL_CONFIG_VFE &&
		cmd != MSM_CAM_IOCTL_PICT_PP_DIVERT_DONE &&
		cmd != MSM_CAM_IOCTL_MCTL_DIVERT_DONE &&
		cmd != MSM_CAM_IOCTL_MCTL_POST_PROC &&
		cmd != MSM_CAM_IOCTL_RESERVE_FREE_FRAME)
...
/* BEGIN Dom_Lin@pegatron [2012/04/12] [Add suspend wake lock] */
		wake_lock(&sync->suspend_wake_lock);
		D("wake_lock(&sync->suspend_wake_lock) complete\n");
/* END Dom_Lin@pegatron [2012/04/12] [Add suspend wake lock] */
...
/* BEGIN Dom_Lin@pegatron [2012/04/12] [Add suspend wake lock] */
	wake_unlock(&p_mctl->sync.suspend_wake_lock);
	D("wake_unlock(&p_mctl->sync.suspend_wake_lock) complete\n");
/* END Dom_Lin@pegatron [2012/04/12] [Add suspend wake lock] */
