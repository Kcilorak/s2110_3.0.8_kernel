...
/* BEGIN Dom_Lin@pegatron [2011/02/02] [Enable auto flash function in 1024 codebase] */
#include "sensors/ov5640_v4l2.h"
/* END Dom_Lin@pegatron [2011/02/02] [Enable auto flash function in 1024 codebase] */
...
#ifdef CONFIG_OV5640
		ov5640_set_flash_mode(STROBE_FLASH_MODE_OFF);
#endif
/* END Dom_Lin@pegatron [2012/05/04] [Reduce the time of opening back camera] */
	case MSM_CAMERA_LED_OFF:
	case MSM_CAMERA_LED_LOW:
	case MSM_CAMERA_LED_HIGH:
		rc = lm3559_set_led(led_state);
		break;
	default:
		rc = -EFAULT;
		break;
...
/* BEGIN Dom_Lin@pegatron [2011/02/02] [Enable auto flash function in 1024 codebase] */
#ifdef CONFIG_OV5640
	if (strobe_ctrl->type == STROBE_FLASH_CTRL_CHARGE) {
		CDBG("strobe_ctrl->mode = %d\n", strobe_ctrl->mode);
		ov5640_set_flash_mode(strobe_ctrl->mode);
	}
#endif
/* END Dom_Lin@pegatron [2011/02/02] [Enable auto flash function in 1024 codebase] */
...
#if defined(CONFIG_LEDS_LM3559) && defined(CONFIG_MSM_CAMERA_FLASH_LM3559)
	CDBG("strobe_ctrl->type = %d, strobe_ctrl->charge_en = %d\n", strobe_ctrl->type, strobe_ctrl->charge_en);
	switch (strobe_ctrl->type) {
	case STROBE_FLASH_CTRL_INIT:
	case STROBE_FLASH_CTRL_CHARGE:
	case STROBE_FLASH_CTRL_RELEASE:
		rc = lm3559_set_strobe((unsigned)strobe_ctrl->type, strobe_ctrl->charge_en);
		break;
	default:
		CDBG("Invalid Strobe Flash State\n");
		rc = -EFAULT;
		break;
	}
#else
...
#endif
/* END BEGIN Dom_Lin@pegatron [2011/02/02] [Init camera strobe flash function in 1024 codebase] */
