...
static int msg_pre_eject(struct fsg_common *common,struct fsg_lun *lun, int num)
{
	return 0;
}

static int msg_post_eject(struct fsg_common *common,struct fsg_lun *lun, int num)
{
	struct android_dev *dev = _android_dev;
	struct usb_composite_dev *cdev = dev->cdev;
	char *eject[2] = { "USB_STATE=EJECT", NULL };
	char **uevent_envp = NULL;
					...
					struct usb_composite_dev *cdev)
{
	...
	static const struct fsg_operations ops = {
		...
		.pre_eject =msg_pre_eject,
		.post_eject=msg_post_eject,
	};
	int err;
