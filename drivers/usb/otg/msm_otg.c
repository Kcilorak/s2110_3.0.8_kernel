...
static int dock_in_gpio_number=MSM8960_DOCK_IN_GPIO;
...
static int msm_otg_notify_chg_type(struct msm_otg *motg)
{
	...
	/*BEGIN Jack1_Huang 20120306 commit for building pass */
	//return pm8921_set_usb_power_supply_type(charger_type);
	return 0;
	/*END Jack1_Huang 20120306 commit for building pass */
}
...
static void msm_otg_pmic_id_work2(struct work_struct *w)
{
	struct msm_otg *motg = container_of(w, struct msm_otg, pmic_id_work2.work);

	if (irq_read_line(motg->pdata->pmic_id_irq))
		motg->pdata->pmic_id(1);
	else
		motg->pdata->pmic_id(0);

}
static void msm_otg_pmic_id_work(struct work_struct *w)
{
	struct msm_otg *motg = container_of(w, struct msm_otg, pmic_id_work.work);
	...
	if (motg->pdata->otg_control == OTG_PMIC_CONTROL)
	{
		...
		if (irq_read_line(motg->pdata->pmic_id_irq))
			schedule_delayed_work(&motg->pmic_id_work2, 200 * HZ/1000);
		else
			motg->pdata->pmic_id(0);
	}
	...
	if (motg->pdata->otg_control == OTG_PMIC_CONTROL)
	{
/*chilung 20120223, move from id detect IRQ start*/	
	       if (aca_id_turned_on)
		       return;;
	       ...
	       if (motg->otg.state != OTG_STATE_UNDEFINED)
		       schedule_work(&motg->sm_work);	
/*chilung 20120223, move from id detect IRQ end*/	
	}
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_A_HOST:
		...
		if (test_bit(ID, &motg->inputs) &&
			...
			if(!gpio_get_value_cansleep(dock_in_gpio_number) && !irq_read_line(motg->pdata->pmic_id_irq))
                        {
			    //msm_otg_notify_charger(motg, 0);
			    msm_hsusb_vbus_power(motg, 1);
		        }
...
#define USB_PMIC_ID_DET_DELAY	(500 * HZ/1000)
...
static void msm_otg_set_vbus_state(int online)
{
	...
	struct msm_otg *motg = the_msm_otg;
       ...
       if (!test_bit(ID, &motg->inputs))
       {
		printk("[OTG] %s, reject set vbus state when usb id is low\n", __func__);	 	
	...
	if (online) {
		...
		if(online == NOTIFY_OTG_CHECK_EVENT) {  // NOTIFY_OTG_CHECK_EVENT = 2
			struct otg_transceiver *otg = &motg->otg;

			motg->chg_state = USB_CHG_STATE_UNDEFINED;
			motg->chg_type = USB_INVALID_CHARGER;
			/*if(ulpi_read(&motg->otg, ULPI_FUNC_CTRL) == -ETIMEDOUT) {
				printk("[OTG] %s, read ULPI_FUNC_CTRL timeout!\n", __func__);
				otg->state = OTG_STATE_UNDEFINED;
			}*/
			msm_otg_reset(otg);
		}
...
static irqreturn_t msm_pmic_id_irq(int irq, void *data)
{
	...
	schedule_delayed_work(&motg->pmic_id_work, USB_PMIC_ID_DET_DELAY);
...
/*Chilung 20120330, add vbus_otg debugfs for debug start*/
static int msm_otg_vbus_show(struct seq_file *s, void *unused)
{
//	struct msm_otg *motg = s->private;
//	struct otg_transceiver *otg = &motg->otg;
        int on;
	if (!vbus_otg) {
		seq_printf(s, "vbus_otg is NULL.\n");
		return 0;
	}

	on=regulator_is_enabled(vbus_otg);

	seq_printf(s, "vbus_otg = %d\n",on);

	return 0;
}

static int msm_otg_vbus_open(struct inode *inode, struct file *file)
{
	return single_open(file, msm_otg_vbus_show, inode->i_private);
}

static ssize_t msm_otg_vbus_write(struct file *file, const char __user *ubuf,
				size_t count, loff_t *ppos)
{
	struct seq_file *s = file->private_data;
	//struct msm_otg *motg = s->private;
	...
	if (!vbus_otg) {
		...
		ret = regulator_enable(vbus_otg);
		if (ret) {
		       seq_printf(s, "unable to enable vbus_otg\n");
		       goto out;
		}
		vbus_is_on = true;
...
const struct file_operations msm_otg_vbus_fops = {
	.open = msm_otg_vbus_open,
	.read = seq_read,
	.write = msm_otg_vbus_write,
	.llseek = seq_lseek,
	.release = single_release,
};
/*Chilung 20120330, add vbus_otg debugfs for debug end*/
...
/*Chilung 20120330, add vbus_otg debugfs for debug start*/
	msm_otg_dentry = debugfs_create_file("vbus_otg", S_IRUGO | S_IWUSR,
		msm_otg_dbg_root, motg,
		&msm_otg_vbus_fops);
...
void msm_otg_early_suspend(struct early_suspend *h)
{
       struct msm_otg *motg = the_msm_otg;

       wake_unlock(&motg->early_suspend_wlock);
}

void msm_otg_late_resume(struct early_suspend *h)
{
	struct msm_otg *motg = the_msm_otg;
	...
	{
#if 0	
	       if(usb_hub_switch(1)!=0)
	       {
                   pr_info("usb_hub_switch fail\n");
	       }
#endif		   
	       if(sdcard_switch(1)!=0)
	       {
                   pr_info("sdcard_switch fail\n");
	       }

	}
	wake_lock(&motg->early_suspend_wlock);
...
static int __init msm_otg_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&motg->early_suspend_wlock, WAKE_LOCK_SUSPEND, "msm_otg_early_suspend");
	INIT_WORK(&motg->sm_work, msm_otg_sm_work);
	INIT_DELAYED_WORK(&motg->pmic_id_work, msm_otg_pmic_id_work);
	INIT_DELAYED_WORK(&motg->pmic_id_work2, msm_otg_pmic_id_work2);
...
/* END-20111214-KenLee-add for westlake charging. */
	...
	if (motg->pdata->phy_type == SNPS_28NM_INTEGRATED_PHY) {
		if (motg->pdata->otg_control == OTG_PMIC_CONTROL &&
			...
			motg->caps = ALLOW_PHY_RETENTION; //remove ALLOW_PHY_POWER_COLLAPSE
	...
	if(get_pega_hw_board_version_status() > 2) {
		...
		dock_in_gpio_number =MSM8960_DOCK_IN_GPIO;
	}

	motg->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN -10;
	motg->early_suspend.suspend = msm_otg_early_suspend;
	motg->early_suspend.resume =msm_otg_late_resume;
	register_early_suspend(&motg->early_suspend);
...
static int msm_otg_pm_suspend(struct device *dev)
{
	...
	if(!gpio_get_value_cansleep(dock_in_gpio_number) && !irq_read_line(motg->pdata->pmic_id_irq))
	{
	       if(sdcard_switch(0)!=0)
	       {
                   pr_info("sdcard_switch fail\n");
	       }
#if 0		   
	       if(usb_hub_switch(0)!=0)
	       {
                  pr_info("usb_hub_switch fail\n");
	       }
#endif		   
...
static int msm_otg_pm_resume(struct device *dev)
{
	...
	if(!gpio_get_value_cansleep(dock_in_gpio_number) && !irq_read_line(motg->pdata->pmic_id_irq))
	{
              /*make not quick to entry suspend*/
		wake_lock_timeout(&motg->wlock,10*HZ);
	}
