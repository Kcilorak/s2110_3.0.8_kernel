...
#define MAXWKPINS 5
int gpio_wakeup_pins[MAXWKPINS];
int* get_gpio_wakeup_pins(void)
{
	return gpio_wakeup_pins;
}
EXPORT_SYMBOL(get_gpio_wakeup_pins);
...
void msm_gpio_show_resume_irq(void)
{
	...
	memset(gpio_wakeup_pins, 0, sizeof(gpio_wakeup_pins));
...
//				__func__, irq);
			gpio_wakeup_pins[(k++)%MAXWKPINS] = i;
			printk("%s: %d(gpio-%d) triggered\n",
				__func__, irq,i);
		}
	}
	spin_unlock_irqrestore(&tlmm_lock, irq_flags);
