...
#if defined(CONFIG_WESTLAKE)
	.detect_client = westlake_msm_fb_detect_panel,
#else
...
int mipi_dsi_cdp_panel_power(int on)
{
...
#if defined( CONFIG_WESTLAKE)
		ret = mipi_dsi_westlake_panel_power(on);
#else
...
struct platform_device mipi_dsi_toshiba_panel_device = {
	.name = "mipi_toshiba",
	.id = 0,
	.dev = {
...
#if defined(CONFIG_WESTLAKE)
	.gpio_num = westlake_dsi2lvds_gpio,
#else
...
static int hdmi_enable_5v(int on)
{
	...
	static struct regulator *reg_8921_boost_5v;	/* HDMI_5V */
	static int prev_on;
	int rc;
	...
	if (!reg_8921_boost_5v)
		reg_8921_boost_5v = regulator_get(&hdmi_msm_device.dev,
			"hdmi_boost_5v");

	if (on) {
		rc = regulator_enable(reg_8921_boost_5v);
		if (rc) {
			pr_err("'%s' regulator enable failed, rc=%d\n",
				"8921_hdmi_boost_5v", rc);
			return rc;
		}
		pr_debug("%s(on): success\n", __func__);
	} else {
		rc = regulator_disable(reg_8921_boost_5v);
		if (rc)
...
/*+Paul, 20111108, add the following codes to configure panel and backlight enable gpio, they are only for lacoste project */
#if defined( CONFIG_WESTLAKE)
	msm_gpiomux_install(msm8960_display_configs,
			ARRAY_SIZE(msm8960_display_configs));	
#endif
/*-Paul, 20111108, add the following codes to configure panel and backlight enable gpio, they are only for lacoste project */
