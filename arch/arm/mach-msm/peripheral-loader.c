...
static int load_image(struct pil_device *pil)
{
	...
	if (ret) {
		dev_err(pil->desc->dev, "Failed to bring out of reset, ret : %d\n", ret);
		goto release_fw;
	}
...
void *pil_get(const char *name)
{
	...
	if (!pil) {
		printk(KERN_ERR "[Wi-Fi] %s : find peripheral error", __func__);
	...
	ret = load_image(pil);
	if (ret) {
		printk(KERN_ERR "[Wi-Fi] %s : load image error, ret %d", __func__, ret);
		retval = ERR_PTR(ret);
		goto unlock;
	}
