...
static void tabla_hs_gpio_handler(struct snd_soc_codec *codec)
{
    ...
    //20120813, Paul, fix TDMS issue 26696 [Music]When play a song then plug a Wired headset, the music would stopped.
    //Headset will be detected as headphone while insertion incompletely, and then push the headset into jack, the GPIO interrupt
    //will be triggered form 1->0->1. If the debounce time is not enough, the gpio_get_value_cansleep function will get 0 value and
    //report the remove uevent. Add debounce time 250ms to avoide this phenomenon.
    msleep(250);
    ...
    //20120813, Paul, fix TDMS issue 26696 [Music]When play a song then plug a Wired headset, the music would stopped.
    //Headset will be detected as headphone while insertion incompletely, and then push the headset into jack, the headset
    //will not be detected again by original code design. The headset will be determined as headphone. We need to detect again while this phenomenon appear.
	if (((tabla->current_plug == PLUG_TYPE_NONE)||(tabla->current_plug == PLUG_TYPE_HEADPHONE)) && insert) {
