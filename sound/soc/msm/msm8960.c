...
static struct tabla_regulator spk_regulators[] = {
	{
		.name = "SPKR_VDD",
	},
	{
...
static int msm8960_enable_supplies(int on)
{
	int ret;
	int i;
	static struct regulator_bulk_data *spk_supplies = NULL;
	...
	if (on) {
		...
		spk_supplies = kzalloc(sizeof(struct regulator_bulk_data) *
					   ARRAY_SIZE(spk_regulators),
					   GFP_KERNEL);
		if (!spk_supplies)
			return -ENOMEM;

		for (i = 0; i < ARRAY_SIZE(spk_regulators); i++)
			spk_supplies[i].supply = spk_regulators[i].name;

		ret = regulator_bulk_get(dev, ARRAY_SIZE(spk_regulators), spk_supplies);
		if (ret != 0) {
			pr_debug("Failed to get supplies: err = %d\n", ret);
			kfree(spk_supplies);
			return ret;
		}

		ret = regulator_bulk_enable(ARRAY_SIZE(spk_regulators), spk_supplies);
		if (ret != 0) {
			pr_debug("Failed to enable supplies: err = %d\n", ret);
			return ret;
		}
	...
	{
		regulator_bulk_disable(ARRAY_SIZE(spk_regulators), spk_supplies);
		regulator_bulk_free(ARRAY_SIZE(spk_regulators), spk_supplies);
		kfree(spk_supplies);
	}
...
static int msm8960_configure_headset_mic_gpios(void)
{
	...
	ret = gpio_request(PM8921_GPIO_PM_TO_SYS(23), "PMIC_JACK_IN_DET");
#else
	struct pm_gpio param = {
		.direction      = PM_GPIO_DIR_OUT,
		.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
